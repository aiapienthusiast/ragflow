...
class CheaperInferenceChat(Base):
    ...
    Cheaper Inference is a hosted gateway that serves each request from one of
    several upstream providers for the requested model, so the standard OpenAI
    client path covers every call. A reply reports the upstream route's model id
    instead of the requested one and carries an extra ``provider`` member; the
    OpenAI client keeps both out of the way of answer and usage extraction, so
    neither needs handling here. The base URL stays tenant-configurable because
    the gateway is also reachable through per-account domains.
    """
