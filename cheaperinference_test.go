...
// The gateway answers from whichever upstream route served the request, so the
// response reports that route's model id -- not the requested one -- and adds a
// "provider" member. Neither may disturb answer or usage extraction.
func TestCheaperInferenceChatAcceptsReroutedResponse(t *testing.T) {
	withSSRFBypass(t)
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		_ = json.NewEncoder(w).Encode(map[string]any{
			"id":       "chat-cheaperinference-rerouted",
			"model":    "upstream-route-model-id",
